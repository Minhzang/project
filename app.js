...
if(searchDateInput) searchDateInput.addEventListener('input',render);
 ...
 reader.onload=ev=>{
  try{
   ...
   const transactions=lines.slice(1).map(line=>{
    ...
    const type=parts[3].trim()==='Thu'?'income':'expense';
    const amount=parseInt(parts[4].replace(/\D/g,''),10)||0;
    return {id:Date.now()+Math.random(),date,description,category,type,amount};
   });
   // replace current data with imported content
   user.transactions=transactions;
   const catSet=new Set([...defaultCategories]);
   transactions.forEach(t=>catSet.add(t.category));
   user.categories=[...catSet];
   editId=null;
   saveUsers();
   render();
   alert('Đã phục hồi dữ liệu từ Excel, các giao dịch hiện tại đã được thay thế');
...
function render(){
 ...
 user.transactions
  ...
  tr.innerHTML=`<td>${t.category}</td><td>${t.type==='income'?'+':'-'}${formatVND(t.amount)}</td><td>${t.description}</td><td>${t.date}</td><td><button data-id="${t.id}" class="edit">Sửa</button><button data-id="${t.id}" class="del">Xóa</button></td>`;
